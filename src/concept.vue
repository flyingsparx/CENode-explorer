<template>
<div class="container">
  <h1>{{concept.name}} <label>Concept <span v-if="concept.parents && concept.parents.length">inherits directly from <span class="badge parent teal white-text" v-for="parent in concept.parents"><router-link :to="{name: 'concept', params: {name:server.name, id: parent.id}}">{{parent.name}}</router-link></span></span></label></h1>
    ...
    <div class="col s12 m4">
      <div class="card light-blue lighten-4">
        <div class="card-content">
          ...
          <ul v-if="concept.children && concept.children.length">
            <li v-for="child of concept.children"><router-link :to="{name: 'concept', params: {name: server.name, id: child.id}}">{{child.name}}</router-link></li>
          </ul>
          <p v-else><i>None</i></p>
        </div>
      </div>
      ...
      <div class="card light-blue lighten-4">
        <div class="card-content">
          ...
          <ul v-if="concept.instances && concept.instances.length">
